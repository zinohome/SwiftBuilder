...
class ApplicationAdmin(SwiftAdmin):
    ...
    async def get_read_form(self, request: Request) -> Form:
        ...
        # 构建子表CRUD - model
        model_table =await self.get_sub_list_table(self.app.get_model_admin('model'), request)
        headerToolbar = [
            {"type": "columns-toggler", "align": "left", "draggable": False},
            {"type": "reload", "align": "right"}
        ]
        model_table.headerToolbar = headerToolbar
        model_table.itemActions = None
        # 增加子表外键过滤
        model_table.api.data['applicaiton_id'] = f"${self.pk_name}"
        #log.debug(table.api)
        model_tabitem = amis.Tabs.Item(title=_('模型管理'), icon='fa fa-square', body=model_table)
        model_tabitem.disabled = False
        formtab.tabs.append(model_tabitem)
        r_form.body = formtab
        return r_form
